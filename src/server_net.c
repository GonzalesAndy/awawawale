...
{
    c->fd = -1; c->name[0] = '\0'; c->buf_len = 0; c->buf[0] = '\0';
}
...
static void server_accept_new(int listen_fd, client_t *clients, int max_clients)
{
    ...
    if (cfd >= 0)
    {
        ...
        for (int i = 0; i < max_clients; ++i)
        {
            if (clients[i].fd == -1)
            {
                clients[i].fd = cfd; clients[i].buf_len = 0; clients[i].name[0] = '\0';
                added = 1; break;
            }
        }
        if (!added) close(cfd); else printf("Accepted connection fd=%d\n", cfd);
    }
}
...
static void server_handle_client_data(client_t *c, client_t *clients, server_state_t *state)
{
    ...
    if (r <= 0)
    {
        if (r == 0) printf("Client fd=%d disconnected\n", c->fd); else perror("recv");
        close(c->fd); client_init(c); return;
    }
    tmp[r] = '\0';
    ...
    c->buf_len += (size_t)r; c->buf[c->buf_len] = '\0';
    ...
    if (rem > 0) memmove(c->buf, line, rem + 1); else c->buf[0] = '\0';
    c->buf_len = rem;
}
...
int server_run(int port)
{
    ...
    if (listen_fd < 0) { perror("create_and_bind"); return 1; }
    printf("Server listening on 0.0.0.0:%d\n", port);

    client_t clients[SERVER_NET_MAX_CLIENTS];
    for (int i = 0; i < SERVER_NET_MAX_CLIENTS; ++i) client_init(&clients[i]);

    server_state_t state; server_init(&state);

    fd_set readset;
    while (1)
    {
        FD_ZERO(&readset); FD_SET(listen_fd, &readset); int maxfd = listen_fd;
        for (int i = 0; i < SERVER_NET_MAX_CLIENTS; ++i)
        {
            if (clients[i].fd != -1) { FD_SET(clients[i].fd, &readset); if (clients[i].fd > maxfd) maxfd = clients[i].fd; }
        }
        int rv = select(maxfd + 1, &readset, NULL, NULL, NULL);
        if (rv < 0) { if (errno == EINTR) continue; perror("select"); break; }
        if (FD_ISSET(listen_fd, &readset)) server_accept_new(listen_fd, clients, SERVER_NET_MAX_CLIENTS);
        for (int i = 0; i < SERVER_NET_MAX_CLIENTS; ++i)
